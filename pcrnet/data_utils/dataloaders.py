...
def load_data(train,target_class=None):
	...
	if target_class:
		target_idx = np.where(all_label == 7)
		return all_data[target_idx[0],:,:], all_label[target_idx[0],:]
	else:
...
class ModelNet40Data(Dataset):
	def __init__(
		...
		if target_class:
			self.data, self.labels = load_data(train,target_class)
		else:
			self.data, self.labels = load_data(train,target_class)
...
class RegistrationData(Dataset):
	def __init__(self, algorithm, data_class=ModelNet40Data(), is_testing=False):
		...
		if self.algorithm == 'PCRNet' or self.algorithm == 'iPCRNet':
			...
			self.transforms = PCRNetTransform(len(data_class), angle_range=45, translation_range=0)
